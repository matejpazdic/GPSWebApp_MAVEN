...
public class DBLoginFinder {
	...
	// FIXME: 07.03.2016 Create setting table in database for selecting the global setting for hashing algorithm
	...
	public boolean isCorrectLogin(String username, String userpass) throws Exception {
		final String  hashedUserPass = StringHasher.toHash(userpass, HASHING_TYPE);
		//TODO Remove this logging this is only for testing
		FileLogger.getInstance().createNewLog(MessageFormat.format("INFO: We are finding user with email: {0} and " +
				"user password is: {1} in hashed for is: {2}", username, userpass, hashedUserPass));

		while (resultSet.next()) {
			String email = resultSet.getString("user_email");
			String pass = resultSet.getString("user_pass");
			if (email.equals(username) && pass.equals(hashedUserPass)) {
				close();
				return true;
			}
